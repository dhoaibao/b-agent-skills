...
dependencies:
  tools:
    - name: brave-search
      description: "Required — web search (brave_web_search) and news search (brave_news_search) for all query types"
    - name: firecrawl
      description: "Required — scrape full page content via firecrawl_scrape; also firecrawl_search, firecrawl_map, firecrawl_crawl for deep coverage. See known limitations below for Codex cloud sandbox."
    - name: context7
      description: "Optional — versioned library/framework documentation for HOWTO/API queries via resolve-library-id and query-docs"
    - name: sequential-thinking
      description: "Optional — structured conflict resolution when sources disagree on a key decision point"
description: >
...
> ⚠️ **Codex cloud sandbox — firecrawl limitation**: Codex cloud sandbox may block outbound
> HTTP connections required by Firecrawl. If `firecrawl_scrape` or other firecrawl tools
> return connection errors, run Codex locally or configure a local Firecrawl proxy. See
> `codex/README.md#known-limitations` for setup instructions. b-research cannot degrade
> gracefully without firecrawl — if unavailable, stop and inform the user.
>
> **Note on parallel scraping in Codex**: the Agent tool (Explore subagent) used for context
> isolation when scraping ≥4 URLs is a Claude Code-specific feature. In Codex, use direct
> parallel `firecrawl_scrape` calls for all URL counts — the ≥4 URL threshold and subagent
> spawn step do not apply.
...
- brave-search missing: "❌ brave-search MCP is not connected. Please check `codex mcp list`.".
- firecrawl missing: "❌ firecrawl MCP is not connected. Please check `codex mcp list`.".
