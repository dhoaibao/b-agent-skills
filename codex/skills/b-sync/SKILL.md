...
description: >
  Sync, update, or bootstrap Codex skills from the b-agent-skills GitHub repo.
...
Syncs Codex skills from the public `b-agent-skills` GitHub repo to `~/.agents/skills/` using git + HTTPS. No extra tools required — just `git`.
...
- `~/.agents/skills/<skill-name>` — symlinks pointing into the `codex/skills/` subfolder of the clone.
...
- `Bash` tool — to run `git` and `codex-sync.sh` commands (built-in, always available)
...
```bash
git clone https://github.com/dhoaibao/b-agent-skills.git ~/.b-agent-skills && bash ~/.b-agent-skills/codex/codex-sync.sh
```
...
2. Re-symlink any new skill folders into `~/.agents/skills/`
3. Remove symlinks for skills that no longer exist in the repo

## What codex-sync.sh does (for reference)

- Pulls latest from `main`
- Scans all `codex/skills/b-*/` folders in the repo.
- Symlinks folders that contain a `SKILL.md` into `~/.agents/skills/`
- Removes stale symlinks for skills deleted from the repo.
- Skips anything without a `SKILL.md`
- Safe to re-run anytime — idempotent.

## Adding a new skill to the repo

1. Create a folder at `codex/skills/` in `b-agent-skills/`: `mkdir codex/skills/my-skill`
2. Add `codex/skills/my-skill/SKILL.md` with proper frontmatter including `policy.allow_implicit_invocation: true`
3. Commit and push
4. Run `codex-sync.sh` on any machine to pick it up
...
Run: `ls ~/.agents/skills/ 2>/dev/null || echo "(none)"`
...
- **BOOTSTRAP**: `git clone https://github.com/dhoaibao/b-agent-skills.git ~/.b-agent-skills && bash ~/.b-agent-skills/codex/codex-sync.sh`
...
If codex-sync.sh exits with error: check the output message.
...
grep -rL 'name:' ~/.agents/skills/*/SKILL.md 2>/dev/null
```
...
Compare the before list (Step 2) vs current `ls ~/.agents/skills/`:
...
After running `codex-sync.sh`, verify installed skills are valid:
...
| Skill not showing in Codex | Check folder has `SKILL.md` with valid `name` + `description` frontmatter and `policy.allow_implicit_invocation: true` |
...
- If `codex-sync.sh` fails, diagnose the error — do not retry blindly.
