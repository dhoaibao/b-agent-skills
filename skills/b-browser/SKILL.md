...
- `serena` - use only when exact source ownership materially improves precision;
...
6. Classify failures as product, harness/setup, environment, auth/session, external-service, flaky/timing, or tool-unavailable. For confirmed product failures needing repository-wide flow evidence, initialize an absent CodeGraph index and use it for that question; use Serena separately only when exact source ownership materially improves precision, and serialize rather than parallelize or batch Serena calls.
