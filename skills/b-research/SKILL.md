...
8. For bounded multi-source research, use this recipe: start with Context7 for versioned library/framework facts; use Firecrawl developer search or bounded public search for primary-source discovery; use Brave for independent corroboration when needed; and use `mcpScript` only for bounded, read-only fan-out with an explicit small call limit and per-source result limit after obtaining the required approval.
