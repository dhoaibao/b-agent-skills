...
- `serena` - lock targets and references; prefer repo-confined `rename_symbol`, `replace_symbol_body`, `insert_before_symbol`, `insert_after_symbol`, `safe_delete_symbol`, and targeted replacements over text patches when reliable.
- `read`/`edit` - unsupported files, Serena fallbacks, prose, comments, and config keys.
...
3. When the transform needs repository-wide impact evidence, initialize an absent CodeGraph index and map that impact; use Serena separately for declarations/references. Use bash with `rg`/`fdfind` for exports, routes, config keys, docs, and generated consumers Serena cannot see.
