...
4. Bounded read-only research may substantiate a specific finding only. Keep the repository review read-only: do not edit, patch, run generators/fixers, or otherwise mutate the worktree. When the disposition is `NEEDS FIXES`, automatically delegate the structured findings back through `intercom` to the implementer session in the same CWD: use `reply` for the active review request, or `send` to that session when no active request exists. Do not provision a session; report a coordination gap if the implementer session or intercom is unavailable.
