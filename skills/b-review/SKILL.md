...
1. Confirm the baseline, compatible reviewer identity, and exact candidate snapshot. It must cover tracked plus relevant untracked/derived content; do not claim requirements coverage without a baseline.
...
4. Bounded read-only research may substantiate a specific finding only. Do not delegate, expand scope, edit, patch, run generators/fixers, or otherwise mutate the worktree.
5. Report blocking findings with location, evidence, impact, violated baseline, minimal correction, and regression check. Corrections must return as a reverified, frozen candidate for another review.
