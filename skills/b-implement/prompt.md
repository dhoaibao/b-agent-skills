...
3. Ask the user directly with `ask_user_question` only for material unresolved choices or blockers. Group related questions up to four; address independent blockers in priority order and wait for each answer. Do not relay normal clarification through a architect.
