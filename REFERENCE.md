...
`/preview-markdown:theme` opens a native Moon/Day selector; `/preview-markdown:render <prompt>` requests a one-response preview, and `/preview-markdown:list` lists the 20 most recent successful previews on the active branch for source copying. This cap affects only the selectable list and does not delete session history. The selected theme is persisted globally in
