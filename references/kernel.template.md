...
- Before handoff, finish discovery and settle one bounded approach; agree before edits. While worker edits, planner does not explore or issue work; after delegation, end turn and wait for worker `send`—no `ask` to wait, sleep, polling, or timeout. Roster/status only selects or handles.
