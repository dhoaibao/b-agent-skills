...
- b-agentic defaults to Off; select `planner`/`worker`. The Worker is the sole worktree writer; use same-CWD roster.
...
- Before handoff, finish discovery and settle one bounded approach; agree before edits when needed. Use `send` for delegation and worker results/reviews; use `ask` only for blockers or clarifications, not to wait. While worker edits, planner does not explore or issue work; after delegation, end the turn and wait for worker `send`—no polling or timeout. Roster/status only selects or handles.
...
- Delivery makes a handoff, result, finding, or approval real. On failed `send`, do not continue, commit, or close on the stale target: `pending`, reply if required, otherwise fresh `list-cwd`, then retry once only if the peer remains live; otherwise pause with an unavailable-peer blocker.
...
- Latest approved plan, handoff, and clarifications set the `b-review` baseline. Only delegated worktree-changing tasks require actual `b-review` of diff/verification before completion; generic review cannot substitute. Findings give location, evidence, impact, violated baseline, smallest correction, and regression check; planner sends findings to worker for verified fix/re-review.
- Two-role blockers call `pending`; inbound asks use `reply` without `list-cwd`/`send`/`ask`; otherwise refresh `list-cwd` then `ask` the assigning planner one focused question using its returned identifier token verbatim (authoritative short ID valid), then wait. Solo/Off blockers ask the user. Planner replies when possible; otherwise asks user; keep work open. In two-role tasks, assigned worker executes its worker-owned work itself; never re-delegates or hands it off to another worker; may ask the assigning planner only for blocker, scope, or external-research decisions. Planner-owned audit/review obtains blocked verification through bounded worker evidence; never planner scripts/tests.
- Decisions/blockers use installed `ask_user_question` and emit privacy-safe `B_AGENTIC_USER_INPUT_NEEDED` on a standalone line; completed tasks passing required delegated review emit `B_AGENTIC_TASK_COMPLETE` instead. Group 1–4 related questions per call, give each question 2–4 concrete options with concise trade-offs, suffix the first recommended option with ` (Recommended)`, and rely on extension's automatic custom-answer row. Never author reserved labels `Other`, `Type something.`, or `Next`. If the package is unavailable or UI is noninteractive, ask one focused plain-text question only and retain the user-input signal. Emit at most one signal and omit both for normal planning, discovery, handoffs, coordination, intermediate updates, and reviews needing fixes.
...
- MCP: CodeGraph, Serena, Context7, Linear, Firecrawl, Brave, and Playwright; nested tools keep policy. Roles do not alter MCP availability/approval; prompt ownership directs execution. Managed Serena/CodeGraph names bypass generic gating only in namespace; protected/outside-project and mismatched tools stay gated.
