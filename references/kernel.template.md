...
- Architect automatically begins same-CWD Executor requests for b-plan, b-research, or b-debug and starts b-review from its candidate handoff; before reporting review completion, automatically returns the structured disposition and findings (including `NEEDS FIXES`, `READY FOR PR`, or `READY WITH FOLLOW-UPS`) through `intercom` to the executor session in the same CWD. The Architect never writes except disposable b-debug probes outside the worktree, removed before reporting. Report a coordination gap if a handoff cannot be sent.
