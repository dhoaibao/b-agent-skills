...
expect(plannerStart.systemPrompt.includes('planner profile (read-only coordinator)') && plannerStart.systemPrompt.includes('safe repository discovery, classified read-only MCP calls') && plannerStart.systemPrompt.includes('worker is the sole worktree writer') && plannerStart.systemPrompt.includes('Never perform implementation edits') && plannerStart.systemPrompt.includes('even when it comes directly from the user') && plannerStart.systemPrompt.includes('never authorization for you to edit') && plannerStart.systemPrompt.includes('Do not emit patches or modifying commands') && plannerStart.systemPrompt.includes('Default to non-blocking Intercom') && plannerStart.systemPrompt.includes('Before every Intercom `send` or `reply`, call `pending` first') && plannerStart.systemPrompt.includes('if it reports an inbound ask, the response must use `reply` for that ask and must not call `send` or `list-cwd`') && plannerStart.systemPrompt.includes('if an inbound ask exists, use `reply`') && plannerStart.systemPrompt.includes('call `list-cwd` again to retrieve the exact session ID') && plannerStart.systemPrompt.includes('call `send` to that exact ID') && plannerStart.systemPrompt.includes('exact session identifier returned verbatim by the immediately preceding authoritative Intercom action') && plannerStart.systemPrompt.includes('display name, alias, or abbreviated prefix') && plannerStart.systemPrompt.includes('successful delivery') && plannerStart.systemPrompt.includes('delivery fails') && plannerStart.systemPrompt.includes('if an inbound ask exists, use `reply` and do not call `send` or `list-cwd`') && plannerStart.systemPrompt.includes('otherwise call a fresh `list-cwd`') && plannerStart.systemPrompt.includes('retry exactly once') && plannerStart.systemPrompt.includes('continue, commit, or close') && plannerStart.systemPrompt.includes('unavailable worker as the blocker') && plannerStart.systemPrompt.includes('explicit exception to avoiding repeated `list-cwd` polling') && plannerStart.systemPrompt.includes('After assigning a task, wait for the worker\'s `send` result') && plannerStart.systemPrompt.includes('instead of polling again') && plannerStart.systemPrompt.includes('Use `send` for task delegation and worker result/review reporting') && plannerStart.systemPrompt.includes('Reserve `ask` for a worker\'s blocker or clarification question to the planner, or for a planner\'s quick-answer need from the worker') && plannerStart.systemPrompt.includes('Keep roster/status calls for selecting a worker') && plannerStart.systemPrompt.includes('Send findings and wait for a revised result') && plannerStart.systemPrompt.includes('Every task delegated to a worker must pass the actual `b-review` skill') && plannerStart.systemPrompt.includes('before you may mark it done, complete, approved, or closed') && plannerStart.systemPrompt.includes('A regular or generic review is insufficient') && plannerStart.systemPrompt.includes('this gate must never be bypassed under any circumstances') && plannerStart.systemPrompt.includes('resolve it when possible by calling Intercom `pending` first and then `reply` to the worker') && plannerStart.systemPrompt.includes('If it cannot be resolved from scope or repository evidence, escalate to the user with one focused question and keep the task open'), 'planner role must enforce delegated waiting, completion review, and unresolved-blocker escalation without blocking analysis');
