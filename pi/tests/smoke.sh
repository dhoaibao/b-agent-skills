...
expect(workerStart.systemPrompt.includes('worker profile (implementation)') && workerStart.systemPrompt.includes('sole worktree writer') && workerStart.systemPrompt.includes('b-debug') && workerStart.systemPrompt.includes('b-refactor') && workerStart.systemPrompt.includes("assigning planner's Intercom session name or id") && workerStart.systemPrompt.includes('Before every Intercom `send` or `reply`, call `pending`') && workerStart.systemPrompt.includes('if an inbound ask exists, use `reply`') && workerStart.systemPrompt.includes('call `list-cwd` again to retrieve the exact session ID') && workerStart.systemPrompt.includes('call `send` to that exact ID') && workerStart.systemPrompt.includes('explicit exception to avoiding repeated `list-cwd` polling') && workerStart.systemPrompt.includes('assigning planner\'s exact session ID') && workerStart.systemPrompt.includes('request must explicitly ask the planner to invoke the actual `b-review` skill') && workerStart.systemPrompt.includes('regular or generic review is insufficient') && workerStart.systemPrompt.includes('Pause all edits') && workerStart.systemPrompt.includes('Resume only when the planner sends actionable findings or a new task') && workerStart.systemPrompt.includes('Planner mode is enforced as read-only'), 'worker role must own implementation, flexible skill routing, and review pauses');
