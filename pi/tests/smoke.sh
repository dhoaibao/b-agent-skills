...
activeTools = ['read', 'bash', 'edit', 'write'];
await roleSessionStartHandler({}, roleContext);
expect(roleStatuses.at(-1)?.value === '<accent>b-agentic: worker</accent>' && activeTools.includes('edit') && activeTools.includes('write'), 'persisted worker restoration must preserve normal active tools after stale planner state');
activeTools = ['read', 'bash', 'edit', 'write', 'recall', 'intercom', 'mcp', 'mcpScript'];
...
roleNotifications.length = 0;
await commands['b-role'].handler('worker', roleContext);
expect(roleStatuses.at(-1)?.value === '<accent>b-agentic: worker</accent>', 'worker status must use the cyan color');
