...
expect(plannerStart.systemPrompt.includes('planner profile (read-only coordinator)') && plannerStart.systemPrompt.includes('safe repository discovery, classified read-only MCP calls') && plannerStart.systemPrompt.includes('worker is the sole worktree writer') && plannerStart.systemPrompt.includes('Never perform implementation edits') && plannerStart.systemPrompt.includes('even when it comes directly from the user') && plannerStart.systemPrompt.includes('never authorization for you to edit') && plannerStart.systemPrompt.includes('Do not emit patches or modifying commands') && plannerStart.systemPrompt.includes('Default to non-blocking Intercom') && plannerStart.systemPrompt.includes('Before every Intercom `send` or `reply`, call `pending`') && plannerStart.systemPrompt.includes('if an inbound ask exists, use `reply`') && plannerStart.systemPrompt.includes('call `list-cwd` again to retrieve the exact session ID') && plannerStart.systemPrompt.includes('call `send` to that exact ID') && plannerStart.systemPrompt.includes('full live session `id` from the immediately preceding `list-cwd` result') && plannerStart.systemPrompt.includes('display name, alias, or abbreviated prefix') && plannerStart.systemPrompt.includes('successful delivery') && plannerStart.systemPrompt.includes('delivery fails') && plannerStart.systemPrompt.includes('if an inbound ask exists, use `reply` and do not retry `send`') && plannerStart.systemPrompt.includes('otherwise call a fresh `list-cwd`') && plannerStart.systemPrompt.includes('retry exactly once') && plannerStart.systemPrompt.includes('continue, commit, or close') && plannerStart.systemPrompt.includes('unavailable worker as the blocker') && plannerStart.systemPrompt.includes('explicit exception to avoiding repeated `list-cwd` polling') && plannerStart.systemPrompt.includes('After assigning a task, wait for the worker\'s result') && plannerStart.systemPrompt.includes('instead of polling again') && plannerStart.systemPrompt.includes('use `ask` only when intentionally waiting for a response') && plannerStart.systemPrompt.includes('Keep roster/status calls for selecting a worker') && plannerStart.systemPrompt.includes('Send findings and wait for a revised result') && plannerStart.systemPrompt.includes('Every task delegated to a worker must pass the actual `b-review` skill') && plannerStart.systemPrompt.includes('before you may mark it done, complete, approved, or closed') && plannerStart.systemPrompt.includes('A regular or generic review is insufficient') && plannerStart.systemPrompt.includes('this gate must never be bypassed under any circumstances') && plannerStart.systemPrompt.includes('ask the user one focused question') && plannerStart.systemPrompt.includes('keep the task open'), 'planner role must enforce delegated waiting, completion review, and unresolved-blocker escalation without blocking analysis');
...
expect(workerStart.systemPrompt.includes('worker profile (implementation)') && workerStart.systemPrompt.includes('sole worktree writer') && workerStart.systemPrompt.includes('b-debug') && workerStart.systemPrompt.includes('b-refactor') && workerStart.systemPrompt.includes('assigning planner as the intended peer') && workerStart.systemPrompt.includes('Before every Intercom `send` or `reply`, call `pending`') && workerStart.systemPrompt.includes('if an inbound ask exists, use `reply`') && workerStart.systemPrompt.includes('call `list-cwd` again to retrieve the exact session ID') && workerStart.systemPrompt.includes('call `send` to that exact ID') && workerStart.systemPrompt.includes('full live session `id` from the immediately preceding `list-cwd` result') && workerStart.systemPrompt.includes('display name, alias, or abbreviated prefix') && workerStart.systemPrompt.includes('successful delivery') && workerStart.systemPrompt.includes('delivery fails') && workerStart.systemPrompt.includes('if an inbound ask exists, use `reply` and do not retry `send`') && workerStart.systemPrompt.includes('otherwise call a fresh `list-cwd`') && workerStart.systemPrompt.includes('retry exactly once') && workerStart.systemPrompt.includes('continue, commit, or close') && workerStart.systemPrompt.includes('unavailable planner as the blocker') && workerStart.systemPrompt.includes('explicit exception to avoiding repeated `list-cwd` polling') && workerStart.systemPrompt.includes('assigning planner\'s exact session ID') && workerStart.systemPrompt.includes('request must explicitly ask the planner to invoke the actual `b-review` skill') && workerStart.systemPrompt.includes('regular or generic review is insufficient') && workerStart.systemPrompt.includes('Pause all edits') && workerStart.systemPrompt.includes('Resume only when the planner sends actionable findings or a new task') && workerStart.systemPrompt.includes('Planner mode is enforced as read-only'), 'worker role must own implementation, flexible skill routing, and review pauses');
