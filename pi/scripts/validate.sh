...
if extension.exists():
    ...
    if preview_extension.exists():
        ...
        for marker in [
            ...
            'getMarkdownTheme', 'Original Markdown source', 'Markdown preview rendered inline',
        ]:
...
if readme.exists():
    text = readme.read_text()
    for marker in [
        'pi-mcp-adapter', 'pi-observational-memory', '@narumitw/pi-usage', '@narumitw/pi-lsp@0.32.0', '@juicesharp/rpiv-ask-user-question',
