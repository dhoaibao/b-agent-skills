...
- First-party extensions: `~/.pi/agent/extensions/b-agentic-preview-markdown.ts`,
  ...
  `b-agentic-worker.ts`, `b-agentic-consult.ts`, and `b-agentic-sync.ts`
...
- Shared extension helpers live under the non-discovered
  `pi/extensions/b-agentic-support/` source directory. The isolated consultant
  tool uses the shared `consultant` preference at
  `~/.pi/agent/b-agentic/role-models.json`.
...
b-agentic defaults to Off for a single-session workflow; the first same-CWD session is not automatically promoted to planner. Explicitly run `/b-role planner` or `/b-role worker` in separate sessions, or start them with `pi --b-role planner|worker`. Run `/b-role off` to return to solo work; tab completion supports all three role names. The role persists with the session and appears in Pi's status bar. `/b-role` selects only a role and does not open a model picker. Explicit `pi --b-role` startup selections and later `/model` changes update planner/worker provider, model, and thinking-level preferences under `~/.pi/agent/b-agentic/role-models.json` without credentials. In planner mode, use `/b-consult-model` to choose a separate consultant provider/model/thinking level for the planner-only `b_consult` tool. It accepts an exact `provider/model [thinking-level]` or a fuzzy model search query, opens a searchable model picker with a visible input and filtered model options in UI mode, supports Pi-style model and thinking-level completions, marks the current active model in the search title and picker, and persists the consultant preference in shared `role-models.json`. The isolated call receives only caller-supplied text, has no tools or repository access, returns bounded natural-language advice, and is advisory rather than evidence. An existing legacy `consult-model.json` file is left untouched and is not migrated.
...
Planner mode is prompt-governed rather than tool-gated: it preserves normal active tools and shared shell, filesystem, MCP, and approval policies. Its injected profile assigns planner skills and directs worker-owned execution to a ready worker discovered through the role-aware roster; it never falls back to implementation. The normal permission extensions continue to protect sensitive paths, dangerous commands, unclassified or unmanaged MCP execution, and outside-project or external/shared actions regardless of role. Worker mode retains normal repository-local automation.

In the planner/worker workflow, the generated ownership mapping gives the planner read-only execution of `b-plan`, external `b-research`, `b-agentic-audit`, `b-review`, and `b-pr-summary`; it delegates `b-design`, `b-implement`, `b-init`, `b-refactor`, `b-debug`, `b-test`, `b-browser`, and `b-commit` to the worker. The planner may still read any skill for planning, delegation, audit, or review: ownership governs execution, not inspection. Planner ownership is limited to read-only decision/planning, external research, audit/review, or release-summary coordination; implementation or mutation, runtime diagnosis, builds/tests, browser/operational verification, commits, mixed, and uncertain work belong to the worker. Direct user wording or no ready worker never permits planner implementation; unknown ownership fails closed to worker ownership and registry validation rejects missing or invalid owners. The planner finishes discovery and settles one bounded handoff before the worker edits; it then waits for the worker result and reviews with `b-review`. The worker is the sole worktree writer and executes the assigned worker-owned task itself; it never re-delegates or hands off that task to another worker. In solo/Off work no coordination applies. Planner-owned audit/review may run non-mutating audit/validation scripts, but gets builds, tests, formatters, generators, or other writing verification through bounded worker evidence.

Coordination is concise natural language, not `B_AGENTIC_*` fields or a state machine. Interactive, user-facing material decisions or blockers in planner or solo/Off work use `ask_user_question`: group 1–4 related questions per call, provide 2–4 concrete options with concise trade-offs per question, suffix the first recommended option with ` (Recommended)`, and rely on the extension's automatic custom-answer row. In planner mode, actual `ask_user_question` tool calls trigger a fixed privacy-safe desktop `User input needed` notification; solo/Off workers do not emit planner notifications. Do not author `Other`, `Type something.`, or `Next`; if the package reports unavailable or the UI is noninteractive, ask one focused plain-text question only. Worker→planner material blockers remain Intercom `ask`/`reply`; native tool-permission prompts remain for browser, external, or privileged actions and are not replaced by questionnaire calls. Before every `send` or `reply`, call `pending`: an inbound ask requires `reply`, not `send` or `list-cwd`; otherwise refresh `list-cwd` and use the identifier token returned verbatim by that immediately preceding authoritative output. An authoritative short ID is valid. Never guess, reconstruct, extend, further abbreviate, or use a stale target, display name, or alias. A message is sent only after delivery succeeds. On failure, do not continue, commit, or close: `pending`, reply if required, otherwise fresh `list-cwd`, then one retry only if the intended peer remains live; otherwise pause with an unavailable-peer blocker. The refresh is the exception to no polling; after delegation the planner ends the turn and waits for worker `send`.
...
checks and reports current tool IDs that are new or absent relative to policy. It does not acquire OAuth tokens, so Linear and Mobbin schema probing remain blocked until an authenticated adapter probe path is available; normal readiness reports their authentication state as unverified. Run it after MCP
