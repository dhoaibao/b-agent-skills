...
The first same-CWD session starts as planner and a provisioned peer becomes worker through the role, planner, and worker extensions' Intercom coordination channel. Run `/b-role` to override the role with `planner`, `worker`, or `off`; tab completion also supports `/b-role planner`, `/b-role worker`, and `/b-role off`. `pi --b-role planner|worker` selects a startup role. `off` is the explicit solo-work escape hatch. The role persists with the session and appears in Pi's status bar. An interactive role selection immediately opens a scoped model picker; the selected provider, model, and thinking level are saved per role under `~/.pi/agent/b-agentic/role-models.json` without credentials. Later `/model` changes update the current role's preference.
