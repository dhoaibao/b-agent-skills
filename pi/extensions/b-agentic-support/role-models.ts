...
function parsePreference(value: unknown): RoleModelPreference | undefined {
  if (!isPlainObject(value) || typeof value.provider !== "string" || typeof value.model !== "string") return undefined;
  if (!value.provider.trim() || !value.model.trim()) return undefined;
  const thinkingLevel = typeof value.thinkingLevel === "string" && THINKING_LEVELS.has(value.thinkingLevel as ThinkingLevel)
    ? value.thinkingLevel as ThinkingLevel
    : undefined;
  return { provider: value.provider, model: value.model, ...(thinkingLevel ? { thinkingLevel } : {}) };
}

export function roleModelsPath(): string {
  return join(process.env.PI_CODING_AGENT_DIR ?? join(homedir(), ".pi", "agent"), "b-agentic", "role-models.json");
}

export function loadRoleModelPreferences(path = roleModelsPath()): RoleModelPreferences {
