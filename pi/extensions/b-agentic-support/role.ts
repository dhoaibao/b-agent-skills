...
- Desktop attention is explicit and privacy-safe: emit at most one mutually exclusive signal—\`B_AGENTIC_TASK_COMPLETE\` only after every required b-review has passed, or \`B_AGENTIC_USER_INPUT_NEEDED\` only for a focused user decision or blocker. Emit the exact signal on its own line and omit both for normal planning, handoffs, coordination, intermediate updates, and reviews needing worker fixes; never emit both signals in one response.
