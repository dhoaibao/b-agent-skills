...
- Use the injected worker roster and Intercom \`list-cwd\` to select one ready same-CWD worker. When none is ready, provision a visible same-CWD session with Intercom or wait for one; never fall back to implementation. Before the handoff, call \`pending\`; if an inbound ask exists, use \`reply\`; if there is nothing to reply to, call \`list-cwd\` again to retrieve the exact worker session ID, then call \`send\` to that exact ID for a concise natural-language handoff containing the goal, scope or invariants, and useful success checks.
- Default to non-blocking Intercom \`send\` for assignments, findings, and approval. Before every Intercom \`send\` or \`reply\`, call \`pending\`; if an inbound ask exists, use \`reply\`; if there is nothing to reply to, call \`list-cwd\` again to retrieve the exact session ID, then call \`send\` to that exact ID. This \`list-cwd\` call is the explicit exception to avoiding repeated \`list-cwd\` polling. After assigning a task, wait for the worker's result instead of polling again; use \`ask\` only when intentionally waiting for a response. Keep roster/status calls for selecting a worker or handling genuine connection needs, not a polling loop. \`reply\` remains available. There is no parsed b-agentic message schema, and the user never relays internal coordination.
- Use only the full live session \`id\` from the immediately preceding \`list-cwd\` result for \`to\`; never use a display name, alias, or abbreviated prefix. Treat a handoff, finding, or approval as sent only after Intercom reports successful delivery. If \`send\` delivery fails, do not retry the stale target or continue, commit, or close: call \`pending\`; if an inbound ask exists, use \`reply\` and do not retry \`send\`; otherwise call a fresh \`list-cwd\`; retry exactly once only if the intended worker is still live, otherwise pause and surface the unavailable worker as the blocker.
- Review the actual diff and verification. Send findings and wait for a revised result, repeating until acceptable. Every task delegated to a worker must pass the actual \`b-review\` skill against the actual diff and verification before you may mark it done, complete, approved, or closed. A regular or generic review is insufficient, and this gate must never be bypassed under any circumstances. If a blocker or decision cannot be resolved from scope or repository evidence, ask the user one focused question and keep the task open; never close on unresolved ambiguity. After approval, tell the worker to remain idle and use b-commit only when the user explicitly requested a commit.`;
...
- Before every Intercom \`send\` or \`reply\`, call \`pending\`; if an inbound ask exists, use \`reply\`; if there is nothing to reply to, call \`list-cwd\` again to retrieve the exact session ID, then call \`send\` to that exact ID. This \`list-cwd\` call is the explicit exception to avoiding repeated \`list-cwd\` polling.
- Use only the full live session \`id\` from the immediately preceding \`list-cwd\` result for \`to\`; never use a display name, alias, or abbreviated prefix. Treat a handoff, result, or review request as sent only after Intercom reports successful delivery. If \`send\` delivery fails, do not retry the stale target or continue, commit, or close: call \`pending\`; if an inbound ask exists, use \`reply\` and do not retry \`send\`; otherwise call a fresh \`list-cwd\`; retry exactly once only if the assigning planner is still live, otherwise pause and surface the unavailable planner as the blocker.
- Keep scope bounded and avoid delegation chains. Retain the assigning planner as the intended peer; after \`pending\` reports nothing to reply to, use \`list-cwd\` again to retrieve that planner's full live session ID before sending any blocker or review message. For a genuine blocker, use \`ask\` with one focused question and wait; a blocker question is not a review request.
- When implementation and useful verification are complete, call Intercom \`pending\`; if an inbound ask exists, use \`reply\`; if there is nothing to reply to, call \`list-cwd\` again to retrieve the assigning planner's exact session ID, then call \`send\` to that exact ID with a concise natural-language review request containing changed paths, verification outcomes, and remaining gaps. The request must explicitly ask the planner to invoke the actual \`b-review\` skill; a regular or generic review is insufficient and never substitutes for it. Pause all edits after sending it.
