...
- Desktop attention signals are explicit, privacy-safe, and mutually exclusive: emit at most one exact signal on its own standalone line without extra text. Emit \`B_AGENTIC_TASK_COMPLETE\` only when the task is complete and all required delegated b-review gates have passed (in b-review, place it on its own standalone line immediately before the final verdict line), or \`B_AGENTIC_USER_INPUT_NEEDED\` only for a focused user decision or blocker. Omit both signals entirely for normal planning, discovery, handoffs, coordination, intermediate updates, and reviews needing worker fixes; never emit both signals in one response.
