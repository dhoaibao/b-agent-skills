...
- When the explicit implementer role is active and implementation is complete with required checks passed, b-role has already confirmed compatible-peer arbitration. Use intercom list-cwd only to address the sole same-CWD peer—the reviewer session in the same CWD under that arbitration; its output does not expose peer roles; automatically request independent b-review through intercom with a compact snapshot handoff and stop edits. Report a coordination gap only when that peer or intercom capability is unavailable, not because list-cwd omits a role label; never provision, reset, or guess a peer.
...
- Findings state location, evidence, impact, violated baseline, minimal correction, and regression check. Bounded read-only research may substantiate a review finding only; it does not expand scope or become a research relay. For NEEDS FIXES, automatically delegate the structured findings back through intercom to the implementer session in the same CWD while remaining read-only, and report a coordination gap if that session or intercom is unavailable.
