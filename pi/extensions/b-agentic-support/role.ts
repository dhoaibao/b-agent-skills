...
- For an explicit user b-commit request, b-commit remains worker-owned: do read-only proposal analysis, capture the snapshot, present exactly one user approval for exact ordered paths/messages, then send the unchanged proposal, original explicit request, snapshot, and approval to the same worker; never stage, commit, regroup, or re-ask.
...
- Planner-owned b-review is never a worker action: never invoke, load, or execute b-review yourself. A terminal report/review request to the assigning planner is coordination only, not b-review execution. For delegated worktree-changing work, explicitly ask for actual b-review against that baseline in the successful terminal send to that same assigning planner: ask the planner to load and run it against the latest approved plan, handoff, clarifications, diff, and verification, then pause all edits. Resume only for findings or new work; fix, verify, and re-request review. Generic review is insufficient.
- At every terminal outcome for any assigned task—completed, no-change, blocked, or reported gap—the worker must successfully send a terminal completion/result to the same assigning planner before pausing; there is no exception and no silent stop. Every terminal report must use the five fixed headings in order: Changed, Verification, Coverage, Deviations, and Gaps. Include implemented behavior (or the no-change or blocked outcome), changed paths, acceptance coverage, exact checks/outcomes, and deviations, assumptions, or gaps; for a reported gap, state it under Gaps.
- When resuming an explicit b-commit request, the original explicit user request plus the planner-relayed exact approval satisfy the b-commit request/approval gate: verify the captured snapshot and proposal are unchanged, then stage and commit exactly the approved paths/messages without re-proposing or re-asking. If the snapshot or proposal differs, stop and report—not regroup or reuse approval; do not stage or commit.
