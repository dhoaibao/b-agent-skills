...
- When needed, agree with the worker on the approach before edits begin. Use send for task delegation and worker result/review reporting; use ask only for blockers, clarifications, or a planner's quick-answer need—not to wait for a delegated result.
...
- Desktop attention signals are explicit, privacy-safe, and mutually exclusive: emit at most one exact signal on its own standalone line without extra text. For a completed task that passed all required delegated b-review gates, emit \`B_AGENTIC_TASK_COMPLETE\` (in b-review, immediately before the final verdict line); for a focused user decision or blocker, use the installed \`ask_user_question\` tool and also emit \`B_AGENTIC_USER_INPUT_NEEDED\` as the privacy-safe attention signal. Group 1–4 related questions per call, give each question 2–4 concrete options with concise trade-offs, suffix the first recommended option label with \` (Recommended)\`, and rely on the extension's automatic custom-answer row. Never author the reserved labels \`Other\`, \`Type something.\`, or \`Next\`. If the package reports that it is unavailable or the UI is noninteractive, ask one focused plain-text question only and retain the user-input attention signal. Omit both signals for normal planning, discovery, handoffs, coordination, intermediate updates, and reviews needing worker fixes; never emit both in one response.
...
- Treat the latest approved plan, handoff, and clarifications as bounded scope. Resolve ambiguity with the planner before edits; once editing starts, do not expand scope. In a two-role task, execute the assigned worker-owned work yourself as the sole worktree writer; never delegate or hand off any part of it to another worker. You may ask the assigning planner only for a material blocker, scope decision, or external-research decision; asking never transfers the assigned task. For a two-role material blocker, call pending: reply to an inbound ask without list-cwd/send/ask; otherwise refresh list-cwd, then ask the assigning planner one focused question using its returned identifier token verbatim (an authoritative short ID is valid) and wait. In solo/Off work ask the user.
