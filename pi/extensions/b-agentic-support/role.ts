...
- When the explicit implementer role is active and implementation is complete with required checks passed, use b_agentic_review_peer to obtain exactly one validated compatible same-CWD reviewer, then automatically request independent b-review through intercom using the successful result's \`handoff.action\`, \`handoff.to\`, and exact \`handoff.messagePrefix\` for the ask carrying a compact snapshot handoff and stop edits. If the selector, compatible reviewer, or intercom capability is unavailable, report the coordination gap and stop; never provision, reset, or guess a peer.
...
- Findings state location, evidence, impact, violated baseline, minimal correction, and regression check. Bounded read-only research may substantiate a review finding only; it does not expand scope or become a research relay. For NEEDS FIXES, use b_agentic_review_peer to validate the exact originating B_AGENTIC_REVIEW_HANDOFF implementer before intercom reply/send; use the successful result's \`returnTarget.to\` as the exact reply/send target, automatically delegate the structured findings back while remaining read-only, and report a coordination gap if the selector or originator is unavailable.
