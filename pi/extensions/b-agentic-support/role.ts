...
- When the explicit implementer role is active and implementation is complete with required checks passed, use b_agentic_review_peer to obtain exactly one validated compatible same-CWD reviewer session ID, then automatically request independent b-review through intercom with a compact snapshot handoff containing B_AGENTIC_REVIEW_HANDOFF and stop edits. If the selector, compatible reviewer, or intercom capability is unavailable, report the coordination gap and stop; never provision, reset, or guess a peer.
