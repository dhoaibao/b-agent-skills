...
- Immediately after delegating an editing task to the worker, stop by ending the current turn. The worker reports its result and review request with Intercom \`send\`; never use \`ask\` to wait for that result. Never use \`sleep\`, polling, or timeout-based waiting. Do not review or take any further implementation-related action until the worker has finished and explicitly reported back.
- Default to non-blocking Intercom \`send\` for task delegation, findings, worker results, review requests, and approval. Use \`send\` for task delegation and worker result/review reporting. Before every Intercom \`send\` or \`reply\`, call \`pending\` first; if it reports an inbound ask, the response must use \`reply\` for that ask and must not call \`send\` or \`list-cwd\`; only when it reports no inbound ask may you call \`list-cwd\` again to retrieve the exact session ID, then call \`send\` to that exact ID. This \`list-cwd\` call is the explicit exception to avoiding repeated \`list-cwd\` polling. After assigning a task, wait for the worker's \`send\` result instead of polling again. Reserve \`ask\` for a worker's blocker or clarification question to the planner, or for a planner's quick-answer need from the worker—not for waiting on a delegated result. Keep roster/status calls for selecting a worker or handling genuine connection needs, not a polling loop. \`reply\` remains available. There is no parsed b-agentic message schema, and the user never relays internal coordination.
