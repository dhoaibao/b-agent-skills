...
import { loadRoleModelPreferences, saveRoleModelPreference, type RoleModelPreference } from "./role-models.ts";

export type ConsultModelPreference = RoleModelPreference & { thinkingLevel: ThinkingLevel };
...
export function loadConsultModelPreference(): ConsultModelPreference | undefined {
  return parsePreference(loadRoleModelPreferences().consultant);
}

export function saveConsultModelPreference(preference: ConsultModelPreference): void {
...
export function isValidConsultToolInput(value: unknown): value is ConsultToolInput {
  ...
  if (keys.some((key) => !["question", "context", "plan"].includes(key))) return false;
