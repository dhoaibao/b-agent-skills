...
const CONSULTANT_TOOL_ALLOWLIST = [
  "read",
  "grep",
  "find",
  "ls",
...
const CONSULTANT_POLICY_EXTENSION = join(
  CONSULTANT_EXTENSION_DIR,
...
function failure(
  error: string,
  extra: Partial<ConsultationDetails> = {},
): ConsultationResult {
  return {
...
function cancelled(): ConsultationResult {
  ...
  return {
    content: [{ type: "text", text: error }],
    details: { status: "cancelled", error },
  };
}
...
function getMcpAdapterExtensionPath(
...
async function createConsultantSession(
  ...
  const modelRuntime = await (
    dependencies.createModelRuntime ?? ModelRuntime.create
  )({ refreshOnCreate: false, signal });
  ...
  const runtimeModel =
    modelRuntime.getModel(preference.provider, preference.model) ?? model;
  const effectiveModel = authBaseUrl
    ? { ...runtimeModel, baseUrl: authBaseUrl }
    : runtimeModel;
  const boundedModel = {
    ...effectiveModel,
  ...
  const unexpectedTools = session
    .getActiveToolNames()
    .filter(
      (name) =>
        !CONSULTANT_TOOL_ALLOWLIST.includes(
          name as (typeof CONSULTANT_TOOL_ALLOWLIST)[number],
        ),
    );
...
function modelsAreEqual(
  a: ConsultSelectableModel | undefined,
  b: ConsultSelectableModel | undefined,
): boolean {
...
function modelLabel(
  ...
  if (
    ...
    badges.push("configured consultant");
...
function getConsultantModels(ctx: ExtensionContext): ConsultSelectableModel[] {
  ...
  if (
    ctx.scopedModels.length > 0 &&
    ctx.model &&
    !models.some((model) => modelsAreEqual(model, ctx.model))
  ) {
...
function searchConsultantModels(
  models: ConsultSelectableModel[],
  query: string,
): ConsultSelectableModel[] {
  const normalized = query.trim();
  return normalized
    ? fuzzyFilter(models, normalized, modelSearchText)
    : [...models];
}
...
class ConsultantModelPicker extends Container implements Focusable {
  ...
  constructor(
    ...
    this.addChild(
      new Text(
        theme.fg(
          "accent",
          theme.bold(
            `Select consultant model (active: ${modelContextLabel(activeModel)})`,
          ),
        ),
        1,
        0,
    ...
    this.addChild(
      new Text(
        theme.fg("dim", "↑↓ navigate · enter select · escape cancel"),
        1,
        0,
      ),
    );
  ...
  private filterModels(): void {
    this.filteredModels = searchConsultantModels(
      this.models,
      this.input.getValue(),
    );
  ...
  private updateList(): void {
    ...
    if (this.filteredModels.length === 0) {
      this.listContainer.addChild(
        new Text(this.theme.fg("warning", "  No matching models"), 1, 0),
      );
    }
...
async function getConsultantModelsForCommand(
  ctx: ExtensionContext,
): Promise<ConsultSelectableModel[]> {
...
async function chooseThinkingLevel(
  ctx: ExtensionContext,
): Promise<ConsultModelPreference["thinkingLevel"] | undefined> {
  if (!ctx.hasUI) return undefined;
  const selected = await ctx.ui.select("Select consultant thinking level", [
    ...CONSULT_THINKING_LEVELS,
  ]);
  return selected &&
    CONSULT_THINKING_LEVELS.includes(
...
async function executeConsultation(
  ...
  const params = (
    legacyCall ? signalOrParams : paramsOrToolCallId
  ) as ConsultToolInput;
  const signal = (legacyCall ? ctxOrSignal : signalOrParams) as
    AbortSignal | undefined;
  const ctx = (
    legacyCall ? dependenciesOrCtx : ctxOrSignal
  ) as ExtensionContext;
  const dependencies = (legacyCall ? maybeDependencies : dependenciesOrCtx) as
    ConsultantDependencies | undefined;
  if (getRole() !== "planner")
    return failure(
      "b_consult is available only in planner role. Switch to planner mode before requesting an advisory consultation.",
    );
  if (!isValidConsultToolInput(params))
    return failure(
      "Invalid b_consult input. Keep question/context/plan within the documented bounds.",
    );
  ...
  if (!preference) {
    return failure(
      "Consultant is not configured. Run /b-consult-model to choose a provider, model, and thinking level; b_consult will not fall back to the active model.",
    );
  }
  const model = ctx.modelRegistry.find(preference.provider, preference.model);
  if (!model) {
    return failure(
      `Configured consultant model ${preference.provider}/${preference.model} is unavailable. Run /b-consult-model and choose an available model; no fallback is attempted.`,
      preference,
    );
  }
  if (!ctx.modelRegistry.hasConfiguredAuth(model)) {
    return failure(
      `Consultant model ${preference.provider}/${preference.model} has no configured authentication. Configure its provider, then run /b-consult-model again; no fallback is attempted.`,
      preference,
    );
  }
  const provider = ctx.modelRegistry.getProvider(preference.provider);
  if (!provider) {
    return failure(
      `Consultant provider ${preference.provider} is unavailable. Run /b-consult-model to choose another configured provider; no fallback is attempted.`,
      preference,
    );
  }
  ...
  try {
    ...
    return failure(
      `Consultant authentication could not be resolved for ${preference.provider}/${preference.model}. Configure the provider and retry; no fallback is attempted.`,
      preference,
    );
  }
  if (signal?.aborted) return cancelled();
  if (!auth.ok) {
    return failure(
      `Consultant authentication is unavailable for ${preference.provider}/${preference.model}. Configure the provider and retry; no fallback is attempted.`,
      preference,
    );
  }
  ...
  let session;
  try {
    session = await createConsultantSession(
      ctx,
      model,
      preference,
      signal,
    ...
    if (signal?.aborted) return cancelled();
    return failure(
      `Consultant request failed for ${preference.provider}/${preference.model}. Retry the consultation; no fallback is attempted.`,
      preference,
    );
  }
  ...
  try {
    await session.prompt(buildConsultantPrompt(params), {
      expandPromptTemplates: false,
    });
    if (signal?.aborted)
      return timedOut
        ? failure(
            `Consultant request timed out after ${CONSULT_TIMEOUT_MS / 1000} seconds. Retry the consultation; no fallback is attempted.`,
            preference,
          )
        : cancelled();
    if (timedOut)
    ...
    const raw = trimBounded(
      session.getLastAssistantText() ?? "",
      MAX_OUTPUT_CHARS,
    );
    if (!raw)
      return failure(
        `Consultant ${preference.provider}/${preference.model} returned no advisory text. Retry the consultation; no fallback is attempted.`,
        preference,
      );
...
const ConsultToolSchema = {
  ...
  properties: {
    question: {
      type: "string",
      description: "The hard solution or plan-review question",
      maxLength: CONSULT_INPUT_LIMITS.question,
    },
    context: {
      type: "string",
      description:
        "Optional caller context to evaluate alongside independent repository evidence",
      maxLength: CONSULT_INPUT_LIMITS.context,
    },
    plan: {
      type: "string",
      description:
        "Optional caller plan to review alongside independent repository evidence",
      maxLength: CONSULT_INPUT_LIMITS.plan,
    },
  },
} as const;
...
export default function bAgenticConsult(pi: ExtensionAPI): void {
  ...
  const thinkingCompletions = (prefix: string) =>
    CONSULT_THINKING_LEVELS.filter((value) =>
  ...
  const chooseModel = async (
    ...
    if (ctx.hasUI) {
      return ctx.ui.custom<ConsultSelectableModel | undefined>(
    ...
    if (matches.length === 0) {
      ctx.ui.notify(
        searchQuery
          ? `No models match "${searchQuery}". Choose a model listed by Pi or configure it first.`
          : "No available models. Configure a provider, then run /b-consult-model provider/model thinking-level.",
        "error",
      );
      return undefined;
    }
    ...
    ctx.ui.notify(
      `Model search "${searchQuery}" matched multiple models: ${matches.map((model) => `${model.provider}/${model.id}`).join(", ")}. Use provider/model to select one.`,
      "error",
    );
    return undefined;
  };
  ...
  pi.registerCommand("b-consult-model", {
    description:
      "Choose the isolated consultant provider, model, and thinking level",
    getArgumentCompletions: (prefix) => {
      const tokens = prefix.trim().split(/\s+/).filter(Boolean);
      if (/\s$/.test(prefix) || tokens.length > 1)
        return thinkingCompletions(tokens.at(-1) ?? "");
      const query = tokens[0] ?? "";
      const matches =
        query &&
        !CONSULT_THINKING_LEVELS.includes(
          query as ConsultModelPreference["thinkingLevel"],
        )
    ...
    handler: async (args, ctx) => {
      ...
      const explicitModel =
        tokens.length > 0 ? parseModelSpec(tokens[0]) : undefined;
      if (explicitModel) {
        if (tokens.length > 2) {
          ctx.ui.notify(
            "Usage: /b-consult-model provider/model [thinking-level]",
            "error",
          );
          return;
        }
        selectedModel = available.find(
          (model) =>
            model.provider === explicitModel.provider &&
            model.id === explicitModel.model,
        );
        if (!selectedModel) {
          ctx.ui.notify(
            `Model ${explicitModel.provider}/${explicitModel.model} is unavailable in the current Pi model scope. Choose a model listed by Pi or configure it first.`,
            "error",
          );
          return;
        }
        ...
        if (
          requestedThinking &&
          CONSULT_THINKING_LEVELS.includes(
            requestedThinking as ConsultModelPreference["thinkingLevel"],
          )
        ) {
          thinkingLevel =
            requestedThinking as ConsultModelPreference["thinkingLevel"];
        ...
        const query = tokens
          .slice(0, thinkingLevel ? -1 : undefined)
          .join(" ")
          .trim();
      ...
      const preference: ConsultModelPreference = {
      ...
      ctx.ui.notify(
        `Consultant model set to ${selectedModel.provider}/${selectedModel.id} (thinking: ${thinkingLevel}) · active model: ${activeModelLabel(ctx)}`,
        "info",
      );
    },
  });
  ...
  pi.registerTool({
    ...
    description:
      "Planner-only bounded, isolated, read-only consultation for a hard solution question or plan review. Independently inspects the current repository with read-only tools and may use the managed MCP research gateway under its normal approval/auth policy; returns bounded natural-language advice.",
    promptSnippet:
      "Planner-only consultation with an explicitly configured isolated model",
    promptGuidelines: [
