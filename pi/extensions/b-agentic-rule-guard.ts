...
const SECRET_PATH =
  /(?:^|[/\s])((?:(?!(?:\.env\.(?:example|sample|template))(?=$|[/\s;&|]))\.env(?:\.[\w-]+)?|[^/\s;&|]+\.pem|credentials\.[^/\s;&|]+|secrets\.[^/\s;&|]+))(?=$|[/\s;&|])/i;
...
export function detectRuleViolations(
  ...
  try {
    ...
    for (const match of command.matchAll(DANGEROUS_GIT))
      violations.push(`forbidden git command: git ${match[1]}`);
    for (const match of command.matchAll(READ_COMMAND)) {
      const path = match[1];
      if (SECRET_PATH.test(path))
        violations.push(`likely-secret read: ${path}`);
  ...
  pi.on("tool_call", (event, ctx) => {
    try {
      ...
      if (violations.length)
        ctx.ui.notify(
          `b-agentic rule guard: ${violations.join("; ")}`,
          "warning",
        );
    } catch {
