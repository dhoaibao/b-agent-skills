...
- Treat `README.md` as the public overview, `REFERENCE.md` as detailed operational documentation, and this file as maintainer guidance.
...
Use `--release` when install, Pi integration, kernel delivery, or release-readiness behavior changes. Run `scripts/b-agentic-audit.sh` through the `b-agentic-audit` skill for repository/design-conformance checks: decision-record traceability, source/generated sync, kernel slimness, and unresolved template tokens. `b-review` is for changed-code review only. Confirm generated assets are synchronized, Pi-specific code remains under `pi/`, and public or maintainer docs reflect changed behavior.
...
- `REFERENCE.md` — detailed operational installation, configuration, lifecycle, safety, MCP, and verification documentation
