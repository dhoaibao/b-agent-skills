...
For deliberate two-session work, use `/b-role` in each session and choose `planner` for the coordinator or `worker` for one same-directory implementation session. You can also type `/b-role planner`, `/b-role worker`, or `/b-role off`. Roles are collaboration prompts, not permission sandboxes: they keep normal tools and task-appropriate skills available. The planner plans, delegates, and reviews but never performs delegated implementation edits; the worker is the sole worktree writer and can sequence implementation, debugging, refactoring, testing, browser, research, design, and init skills as needed. Assignments, review requests, findings, and approval use natural-language Intercom `send`; the worker targets the assigning planner, and `ask` is reserved for genuine blockers. The worker pauses after requesting review and resumes only for findings or a new task, repeating until approval. Intercom messages have no required marker or field order. Use `off` for the normal single-session workflow. See `pi/configs/README.md` for role details and CLI flags.
