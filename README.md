...
| Mobbin | Comparable shipped UI patterns for bounded design-reference evidence | Optional per task; OAuth may be required when used | Configured lazily and restricted to `mobbin_search_screens`, `mobbin_search_flows`, and `mobbin_search_sections`; authentication state is unverified, so run `/mcp-auth mobbin` if needed |
...
| `pi-intercom` | Provides planner/worker coordination | Default for the optional multi-role workflow | Installed automatically |
