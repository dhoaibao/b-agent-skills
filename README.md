...
Planner and worker are b-agentic's default two-session workflow: the first same-directory session is the read-only planner and a provisioned peer becomes the sole worktree-writing worker. `/b-role planner`, `/b-role worker`, and `/b-role off` override that selection; `off` is the explicit solo-work escape hatch. The planner uses planning, research, review, and PR-summary skills; the worker owns implementation, debugging, refactoring, tests, browser checks, design/init writes, and explicit-user-request commits. Role-aware Intercom discovery identifies the worker; assignments, review requests, findings, and approval use natural-language `send`, while `ask` is reserved for genuine blockers. The worker pauses after requesting review and resumes only for findings or a new task, repeating until approval. Selecting a role interactively continues to a model picker, and each role's model and thinking-level preference is stored in user-local b-agentic state. See `pi/configs/README.md` for role details and CLI flags.
