...
b-agentic defaults to Off for a single-session workflow: it does not automatically promote the first session to planner. For the two-role workflow, explicitly start one session as planner and another as worker with `/b-role planner|worker` or `pi --b-role planner|worker`; `/b-role off` returns to solo work. The planner uses planning, research, review, and PR-summary skills; the worker owns implementation, debugging, refactoring, tests, browser checks, design/init writes, and explicit-user-request commits. Role-aware Intercom discovery coordinates the explicitly selected roles; assignments, review requests, findings, and approval use natural-language `send`, while `ask` is reserved for genuine blockers. The worker pauses after requesting review and resumes only for findings or a new task, repeating until approval. `/b-role` selects only a role; it does not open a model picker. Role model and thinking-level preferences remain user-local for explicit `pi --b-role` startup selections and `/model` changes. See `pi/configs/README.md` for role details and CLI flags.
